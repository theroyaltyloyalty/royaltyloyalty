...
contract FlexRoyaltyReceiver is IFlexRoyaltyReceiver, ERC165 {
    ...
    {
        return
            interfaceId == type(IFlexRoyaltyReceiver).interfaceId ||
            super.supportsInterface(interfaceId);
    }

    function flexRoyaltyInfo(
        uint256 tokenId,
        uint256 salePrice,
        bool isSeller
        ...
        if (isSeller) {
            royaltyAmount =
                (salePrice * royalty.sellerFraction) /
                _feeDenominator();
        } else {
            royaltyAmount =
                (salePrice * royalty.buyerFraction) /
                _feeDenominator();
        }
    ...
    function onRoyaltyReceived(
        ...
        if (currency == address(0)) {
            ...
            royaltyRespected =
                ...
                (currentBalance - prevBalance) >= royaltyAmount;
        }

        if (royaltyRespected) {
            address royaltyReceiver = _tokenRoyaltyInfo[_tokenId].receiver;
            if (currency == address(0)) {
                _royaltyAccrued[royaltyReceiver][currency] += msg.value;
            } else {
                _royaltyAccrued[royaltyReceiver][currency] += (currentBalance -
                    prevBalance);
            }
        }
    ...
    function _setDefaultRoyalty(
        ...
        _defaultRoyaltyInfo = FlexRoyaltyInfo(
            receiver,
            buyerNumerator,
            sellerNumerator,
            currency
    ...
    function _setTokenRoyalty(
        ...
        _tokenRoyaltyInfo[tokenId] = FlexRoyaltyInfo(
            receiver,
            buyerNumerator,
            sellerNumerator,
            currency
