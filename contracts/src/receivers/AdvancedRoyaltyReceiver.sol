...
abstract contract AdvancedRoyaltyReceiver is IRoyaltyReceiver, ERC165 {
    ...
    {
        return
            interfaceId == type(IRoyaltyReceiver).interfaceId ||
            super.supportsInterface(interfaceId);
    }

    function buyerRoyaltyInfo(uint256 tokenId, uint256 salePrice)
        public
        view
        virtual
        returns (address, uint256)
    {
        ...
        uint256 royaltyAmount = (salePrice * royalty.buyerFraction) /
            _feeDenominator();

        return (address(this), royaltyAmount);
    }

    function sellerRoyaltyInfo(uint256 tokenId, uint256 salePrice)
        public
        view
        virtual
        returns (address, uint256)
    {
        ...
        uint256 royaltyAmount = (salePrice * royalty.sellerFraction) /
            _feeDenominator();
        ...
        returns (address, uint256)
    {
        ...
        uint256 royaltyAmount = (salePrice *
            (royalty.sellerFraction + royalty.buyerFraction)) /
            _feeDenominator();
    ...
    function claimRoyalty(address receiver)
        external
        virtual
    ...
    function _setDefaultRoyalty(
        address receiver,
        uint48 buyerNumerator,
        uint48 sellerNumerator
    ) internal virtual {
        require(
            (buyerNumerator <= _feeDenominator()) &&
                (sellerNumerator <= _feeDenominator()),
            'ERC2981: royalty fee will exceed salePrice'
        );
        require(receiver != address(0), 'ERC2981: invalid receiver');

        _defaultRoyaltyInfo = AdvancedRoyaltyInfo(
            receiver,
            buyerNumerator,
            sellerNumerator
        );
    ...
    function _setTokenRoyalty(
        ...
        _tokenRoyaltyInfo[tokenId] = AdvancedRoyaltyInfo(
            receiver,
            buyerNumerator,
            sellerNumerator
        );
